...
int oculusgo_init(struct android_app *app)
{
    ...
    DLOG("eglGetConfigs() -> numConfigs = %d", numConfigs);
    ...
    VLOG("eglMakeCurrent(Display, Surface, Surface, Context)");
	...
	// todo: create non 1024x1024, but proper half of width x height
    for (int i = 0; i < 2; i++)
	...
	VLOG("Let's enter VR mode simulation");
    ...
    void* gl2es = dlopen("libgl2es.so", RTLD_NOW);
...
// vr-begin
void vrBegin() // TODO: rename to vrBegin(),etc.
{
...
static bool ovrFramebuffer_Create(ovrFramebuffer *frameBuffer, GLenum colorFormat, int width, int height, int multisamples)
{
    ...
    for (int i = 0; i < frameBuffer->TextureSwapChainLength; i++)
    {
		...
		VLOG("renderFramebufferStatus = %d", renderFramebufferStatus);
        if (multisamples > 1) {
	...
	VLOG("end of ovrFramebuffer_Create");
    return 1;
}
